...
class Product_model extends Core_Model
{
    ...
    public function filter_products($query_string_array = null, $category_id = null)
    {
        ...
        if (!empty($array_selected_filters)) {
            ...
            foreach ($array_selected_filters as $filter) {
                ...
                $this->db->where('custom_fields_product.product_filter_key', $filter->key);
                $this->db->group_start();
        ...
        //product type array
        $array_product_type = @explode(',', $product_type);
        if (!empty($array_product_type) && !empty($array_product_type[0])) {
            $this->db->group_start();
            $this->db->where_in("products.product_type", $array_product_type);
            $this->db->group_end();
        }
        if ($p_min != "") {
    ...
    {
        $this->build_query();
        return $this->db->where('products.category_id', clean_number($category_id))->where('products.id !=', clean_number($product_id))->order_by('rand()')->limit(5)->get('products')->result();
    }
