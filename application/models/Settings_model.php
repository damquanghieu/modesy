...
class Settings_model extends CI_Model
{
    ...
    public function update_stripe_settings()
    {
        $data = array(
            ...
            'stripe_locale' => trim($this->input->post('stripe_locale', true))
        );
    ...
    */
    ...
    //check version
    public function check_version($general_settings)
    {
        if (!empty($general_settings) && $general_settings->version != '1.7.1') {
            $t = array();
            ...
            foreach ($languages as $language) {
                foreach ($t as $key => $value) {
                    if (empty($this->db->where('lang_id', $language->id)->where('label', $key)->get('language_translations')->row())) {
            ...
            $this->db->where('id', 1);
            $this->db->update('general_settings', ['version' => '1.7.1']);
        }
    }
